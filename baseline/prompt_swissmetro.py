...
def prompt(row):
    # p1: Task description
    p1 = "You are an expert in the field of travel mode choice. Your task is to predict a person's travel mode choice from Train, Car, and Swissmetro based on travel time, travel cost information of each mode, the person's attributes, logic behind their vehicle choices and the similar trips."
    ...
    formatted_results = "Read the person's previous choices and deduce the logic behind their vehicle choices. The person's previous choices:\n"
    formatted_results += "".join(situations)
    formatted_results += "Read similar examples from others to get a general understanding. The similar trips:\n"
...
if __name__ == "__main__":
    df = pd.read_csv("data/Swissmetro/test.csv")
    row = df.iloc[200]
