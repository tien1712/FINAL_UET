...
raw_api_keys = [os.getenv(f"GOOGLE_API_KEY_{i}") for i in range(1, 7)]
...
def safe_parse_json(raw: str):
    ...
    try:
        ...
        try:
            # tìm pattern prediction: <label>
            m = re.search(r"prediction\s*[:=\-\s]+(Train|Swissmetro|Car)", text, flags=re.IGNORECASE)
            if not m:
                # hoặc chỉ có label đứng riêng lẻ
                m = re.search(r"\b(Train|Swissmetro|Car)\b", text, flags=re.IGNORECASE)
            if m:
...
class PerKeyRateLimiter:
    def __init__(self, key: str, per_minute_limit: int = 10, per_day_limit: int = 250):
        self.key = key
...
class ApiKeyPool:
    def __init__(self, api_keys_list, per_minute_limit: int = 10, per_day_limit: int = 250):
...
async def run_all(rows, result_df, result_path: str, api_keys_list, max_workers: int = None):
    ...
    pool = ApiKeyPool(api_keys_list, per_minute_limit=10, per_day_limit=250)
